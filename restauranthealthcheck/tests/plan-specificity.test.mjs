...
{
  // ถามเขาทั้งชุดแล้วเอาไปแค่บวกเป็นคะแนน โดยเนื้อหาไม่เคยโผล่ในรายงานเลย
  ...
  // ทีม CP ขอให้ตัดคำถามเรื่องหาพาร์ทเนอร์ออก เพราะยังไม่พร้อมเข้าไปช่วยเรื่องนี้
  // ถามในสิ่งที่ตอบไปแล้วไม่มีใครทำอะไรต่อได้ คือการสร้างความคาดหวังลอย ๆ
  const partner = await page.evaluate(() => QUESTIONS
    .map((q, i) => ({ i, q: q.q }))
    .filter((x) => /พาร์ทเนอร์|หุ้นส่วนเพื่อโต/.test(x.q)));
  t('ไม่มีคำถามชวนหาพาร์ทเนอร์แล้ว', partner.length === 0,
    partner.map((x) => `ข้อ ${x.i}: ${x.q}`).join(' · '));
